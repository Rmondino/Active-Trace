...
import { useCohortes, useCreateCohorte, useUpdateCohorte, useDeleteCohorte } from '../hooks/useCohortes'
import { useCarreras } from '../hooks/useCarreras'
import { LoadingSpinner } from '@/shared/components/LoadingSpinner'
import { ErrorMessage } from '@/shared/components/ErrorMessage'
import type { Cohorte, CohortePayload } from '../types/estructura'

const columnHelper = createColumnHelper<Cohorte>()
...
export function CohortesPage() {
  ...
  const [formData, setFormData] = useState<CohortePayload>({ carrera_id: '', codigo: '', anio: new Date().getFullYear(), estado: 'activo' })

  const carrerasQuery = useCarreras()
  const { data, isLoading, error } = useCohortes(carreraFilter || undefined)
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('carrera_nombre', { header: 'Carrera' }),
  ...
  const openCreate = () => {
    setEditing(null)
    setFormData({ carrera_id: carreraFilter || '', codigo: '', anio: new Date().getFullYear(), estado: 'activo' })
    setModalOpen(true)
  }

  const handleEdit = (cohorte: Cohorte) => {
    setEditing(cohorte)
    setFormData({ carrera_id: cohorte.carrera_id, codigo: cohorte.codigo, anio: cohorte.anio, estado: cohorte.estado })
    setModalOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-end gap-4 rounded-lg border border-gray-200 bg-white p-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Filtrar por carrera</label>
          <select value={carreraFilter} onChange={e => setCarreraFilter(e.target.value)}
      ...
      {isLoading ? <LoadingSpinner /> : error ? <ErrorMessage message="Error al cargar cohortes" /> : (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {table.getRowModel().rows.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-500">No hay cohortes registrados</td></tr>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={closeModal}>
          <div className="bg-white rounded-lg w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4">{editing ? 'Editar cohorte' : 'Nuevo cohorte'}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Carrera</label>
                <select value={formData.carrera_id} onChange={e => setFormData(p => ({ ...p, carrera_id: e.target.value }))}
              ...
              <div>
                ...
                <input type="number" value={formData.anio} onChange={e => setFormData(p => ({ ...p, anio: Number(e.target.value) }))}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
              </div>
              <div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button onClick={handleSubmit} disabled={isPending || !formData.codigo || !formData.carrera_id}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteId(null)}>
          <div className="bg-white rounded-lg w-full max-w-sm mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-600 mb-4">¿Estás seguro de eliminar este cohorte? Esta acción no se puede deshacer.</p>
