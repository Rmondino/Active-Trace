...
import { useCarreras, useCreateCarrera, useUpdateCarrera, useDeleteCarrera } from '../hooks/useCarreras'
import { LoadingSpinner } from '@/shared/components/LoadingSpinner'
import { ErrorMessage } from '@/shared/components/ErrorMessage'
import type { Carrera, CarreraPayload } from '../types/estructura'

const columnHelper = createColumnHelper<Carrera>()
...
export function CarrerasPage() {
  ...
  const [formData, setFormData] = useState<CarreraPayload>({ codigo: '', nombre: '', estado: 'activo' })
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <LoadingSpinner /> : error ? <ErrorMessage message="Error al cargar carreras" /> : (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {table.getRowModel().rows.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-500">No hay carreras registradas</td></tr>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={closeModal}>
          <div className="bg-white rounded-lg w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold mb-4">{editing ? 'Editar carrera' : 'Nueva carrera'}</h3>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteId(null)}>
          <div className="bg-white rounded-lg w-full max-w-sm mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-600 mb-4">¿Estás seguro de eliminar esta carrera? Esta acción no se puede deshacer.</p>
