# RBAC — Guard require_permission y PermissionService
...
El guard `require_permission` SHALL aceptar un `owner_check` opcional que es una función con firma `(request: Request, user: User) -> bool`.
